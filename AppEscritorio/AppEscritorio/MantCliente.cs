...
namespace AppEscritorio
{
    public partial class MantCliente : Form
    {
        ...
        {  
            ...
            adapt=new SqlDataAdapter("select * from TipoCliente",con);  
            adapt.Fill(dt);  
        ...
        private void btnCrear_Click(object sender, EventArgs e)
        {
            ...
            {  
                cmd = new SqlCommand("insert into TipoCliente(Nombre,Descripcion) values(@nombre,@descripcion)", con);  
                con.Open();  
        ...
        private void btnActualizar_Click(object sender, EventArgs e)
        {
            if (txtNombre.Text != "" && txtDescripcion.Text != "")  
            {  
                cmd = new SqlCommand("update TipoCliente set Nombre=@nombre,Descripcion=@descripcion where ID=@id", con);  
                con.Open();  
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            if (ID != 0)
            {
                cmd = new SqlCommand("delete TipoCliente where ID=@id", con);
                con.Open();
                cmd.Parameters.AddWithValue("@id", ID);
                cmd.ExecuteNonQuery();
                con.Close();
